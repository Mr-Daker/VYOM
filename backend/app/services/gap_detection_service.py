...
class GapDetectionService(BaseService):
    ...
    def _get_direct_and_transitive(self, graph: Dict[UUID, List[UUID]], target_id: UUID, all_comps: Dict[UUID, object]) -> Tuple[List[UUID], List[UUID], List[str]]:
    ...
    def detect_gaps(self, session_id: UUID) -> GapDetectionResponse:
        ...
        for s in active_students:
            ...
            def evaluate_prereq(cid, relationship):
                ...
                stale = snap.stale if snap else False
                ...
                if mastery_source == "none" or eff_state == MasteryState.UNKNOWN:
                    if raw_m and raw_m.last_updated > session.date:
